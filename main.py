...
async def push_report(request: Request):
    """Receive nightly analysis report from Cowork scheduled task and push to LINE."""
    ...
    if not LINE_MY_USER_ID:
        raise HTTPException(status_code=500, detail="LINE_MY_USER_ID not configured")

    if not message:
        raise HTTPException(status_code=400, detail="No message provided")

    # LINE text messages have a 5000-char limit; split if needed
    chunks = [message[i:i+4900] for i in range(0, len(message), 4900)]
    msgs = [text_msg(chunk) for chunk in chunks[:5]]  # max 5 messages per push

    try:
        send_push(LINE_MY_USER_ID, msgs)
        logger.info("Push report sent to %s (%d chunk(s))", LINE_MY_USER_ID, len(msgs))
